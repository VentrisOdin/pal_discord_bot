...
# Visual polish: per-source colors
COLORS = {
...
class Disasters(commands.Cog):
    ...
    async def cog_load(self):
        ...
        if not self.poll_disasters.is_running():
            ...
            logging.info(
                "Disasters: poll loop started (every %s min).",
    ...
    async def _get_bool_setting(self, key: str, default: bool) -> bool:
        val = await self._get_setting(key, "true" if default else "false")
    ...
    async def _safe_send(self, channel: discord.TextChannel, *, content=None, embed: discord.Embed | None = None):
        try:
            ...
            logging.warning("Disasters: missing permissions to send in #%s (%s)", channel.name, channel.id)
    ...
    async def disasters_now(self, interaction: discord.Interaction):
        ...
        try:
            ...
            use_usgs = await self._get_bool_setting("ENABLE_USGS", True)
            use_rw = await self._get_bool_setting("ENABLE_RELIEFWEB", True)
            use_eonet = await self._get_bool_setting("ENABLE_EONET", True)
            use_gdacs = await self._get_bool_setting("ENABLE_GDACS", True)

            calls = []
            if use_usgs: calls.append(self.fetch_usgs(min_mag))
            if use_rw: calls.append(self.fetch_reliefweb(rw_limit, rw_app))
            if use_eonet: calls.append(self.fetch_eonet())
            if use_gdacs: calls.append(self.fetch_gdacs())
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        e.add_field(name="Sources", value=f"USGS:{use_usgs} RW:{use_rw} EONET:{use_eonet} GDACS:{use_gdacs}", inline=False)
    ...
    async def poll_disasters(self):
        try:
            # refresh dynamic interval every cycle
